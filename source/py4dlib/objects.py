...
#     @staticmethod
#     def __rinsert(obj, iterable, keylist):
#         """ Insert obj into a dict based hierarchy, using keylist as guide.
#         
#         This is used internally by ``asdict()`` to insert new 
#         subnodes recursively as needed.
#         
#         It is assumed:
#             - that keylist ist sorted correctly, in descending 
#               order, from lowest depth level to highest.
#             - that subnode names are unique, otherwise subnodes 
#               will be descended into in order of occurrence.
#             - that obj will always be of the same type
#               (in case this method is called from a loop).
#     
#         @param iterable: the iterable to insert to.
#         @type iterable: ``dict`` or ``list``
#         @param keylist: 
#             a list of parent node names in depth descending order, 
#             e.g. if N == depth level, then [N, N+1, N+2, ...]
#         @type keylist: ``list<string>``
#         @return: last (sub)iterable that was modified.
#         @rtype: ``dict`` or ``list``.
#         """
#         #if DEBUG: print "iterable = %r" % (iterable)
#         if isinstance(iterable, dict):
#             curkey = keylist[0]
#             if curkey not in iterable:
#                 klen = len(keylist)
#                 if klen == 0:
...
#                     val = []
#                 iterable[curkey] = val
#                 if DEBUG: print("inserting value %r for key %r" % (val, curkey))
#                 if klen == 0:
#                     return iterable
#             return ObjectHierarchy.__rinsert(obj, iterable[curkey], keylist=keylist[1:])
#         elif isinstance(iterable, list):
#             if len(keylist) == 0:
#                 # sub list is empty, just append obj
#                 if obj not in iterable:
#                     if isinstance(obj, list):
#                         filtered = []                    
#                         for val in obj:
#                             # filter objects that are already in iterable
#                             if val not in iterable:
#                                 filtered.append(val)
#                             else:
#                                 if DEBUG: print("value %r already exists in %r: skipping..." % (val, iterable))
#                                 pass # IGNORE:W0107
#                             if DEBUG: print("extending %r with %r (%d filtered)" % (iterable, filtered, (len(obj)-len(filtered))))
#                             iterable.extend(filtered)
#                     else:
#                         if DEBUG: print("appending value %r at end of %r" % (obj, iterable))
#                         iterable.append(obj)
#                 else:
#                     if DEBUG: print("value %r already exists in %r: skipping..." % (obj, iterable))
#                     pass # IGNORE:W0107
#                 return iterable
#             curkey = keylist[0]
#             for entry in iterable:
#                 if isinstance(entry, dict) and curkey in entry:
#                     return ObjectHierarchy.__rinsert(obj, entry, keylist)
#             if len(keylist) == 1:
#                 # end of sublist reached but no sub-dict found
#                 # as list member, so create a new subtree by consuming 
#                 # the last key
#                 if isinstance(obj, list):
#                     subhierarchy = {curkey: obj}
#                 else:
#                     subhierarchy = {curkey: [obj]}
#                 # test if the sublist contains curkey and if so replace it 
#                 # with a dict node
#                 if curkey in iterable:
#                     idx = iterable.index(curkey)
#                     if DEBUG: print("replacing obj in %r with %r" % (iterable, subhierarchy))
#                     iterable[idx] = subhierarchy
#                 else:
#                     if DEBUG: print("inserting hierarchy %r at end of %r" % (subhierarchy, iterable))
#                     iterable.append(subhierarchy)
#             return iterable
#         else:
#             # should never arrive here, actually 
#             raise AssertionError('should never arrive here')
# 
#     def asdict(self, stopobj=None, filtertype=None):
#         """
#         Convert hierarchy under self.root to a dict-in-dict based representation.
#             
#         For each node a dict is inserted which contains simple lists representing
#         all children under the this node. The keys are the names of the nodes as returned
#         by GetName() (c4d.BaseObject).
#         """
#         srcobj = self.root
#         #srcname = srcobj.GetName()
#         hierarchy = {}
#         for op, lvl in ObjectIterator(srcobj):
#             if filtertype is None or (filtertype and op.GetType() == filtertype):
#                 opname = op.GetName()
#                 #if DEBUG: print "lvl = %d, op = %s" % (lvl, opname)
#                 plist = []
#                 opp = op
#                 i = lvl
#                 while opp:
...
#                         oppname = opp.GetName()
#                         i -= 1
#                         pentry = ObjectEntry(opp, i, plist)
#                         #pentry = {oppname: {'op': opp, 'children': None}}
#                         #pentry = oppname
#                         plist.append(pentry)
#                 plist.reverse()
#                 #if DEBUG: print "%s has %d parents: %r" % (opname, len(plist), plist)
#                 entry = ObjectEntry(op, lvl, plist)
#                 #entry = {opname: {'op': op, 'children': []}}
#                 ObjectHierarchy.__rinsert(entry, hierarchy, plist)
#             if lvl > self.maxlvl:
#                 self.maxlvl = lvl
